			...
			ResultSet rs = st.executeQuery("SELECT * FROM lojas");		
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	                loja[i] = new Lojas(rs.getString("nome"), rs.getString("notaConsumidor"),rs.getString("NumReclamacoes"), rs.getString("NumReclamacoesResp"), rs.getString("porcProblemasResolv"));
	             }
	          }
	          st.close();
		} catch (Exception e) {
