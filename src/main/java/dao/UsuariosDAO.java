...
public class UsuariosDAO {
			...
			ResultSet rs = st.executeQuery("SELECT * FROM Usuario");		
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	                usuario[i] = new Usuarios(rs.getString("id"), rs.getString("email"), rs.getString("senha"));
	             }
	          }
	          st.close();
		} catch (Exception e) {
	...
	public Usuarios getUsuario(String email) {
		...
		try {
			Statement st = conexao.createStatement(ResultSet.TYPE_SCROLL_INSENSITIVE,ResultSet.CONCUR_READ_ONLY);
			ResultSet rs = st.executeQuery("SELECT * FROM Usuario WHERE Usuario.username LIKE " + email);		
	         if(rs.next()){
	             rs.last();
	             usuarios = new Usuarios[rs.getRow()];
	             rs.beforeFirst();

		         usuarios[0] = new Usuarios(rs.getString("id"), rs.getString("email"), rs.getString("senha"));
		         usuario = usuarios[0];
	          }
	          st.close();
		} catch (Exception e) {
